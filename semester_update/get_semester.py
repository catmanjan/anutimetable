...
if not os.path.exists(os.path.join(os.path.dirname(os.path.realpath(__file__)),  str(year)+'.json')):
    # if checked, no need to run.
    with open(os.path.join(os.path.dirname(os.path.realpath(__file__)),  str(year)+'.json'), 'w'): 
        # Get ANU offical academic calendar
        # url= 'https://www.anu.edu.au/directories/university-calendar/' + str(year)+ '/calendar.ics'
        url='http://localhost:5500/semester_update/calendar.ics'
        ...
        for event in all_event:
            if event.summary == 'Semester 1 begins':
                data['Semester 1']['start'] = event.start.date()
            if event.summary=='Semester 2 begins':
                data['Semester 2']['start']=event.start.date()
            if event.summary=='Semester 1 ends':
                data['Semester 1']['end']=event.start.date()
            if event.summary=='Semester 2 ends':
                data['Semester 2']['end']=event.start.date()
