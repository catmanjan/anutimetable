...
with TemporaryDirectory() as temp_dir:
    ...
    copy(os.path.join(os.path.dirname(__file__),
         'scraper_template.py'), os.path.join(temp_dir, 'scraper.py'))

    inplace_change(os.path.join(temp_dir, 'scraper.py'),
                   '<%= year%>', str(today.year))
    inplace_change(os.path.join(temp_dir, 'index.html'), '<%= year%>', str(today.year))
    inplace_change(os.path.join(temp_dir, 'timetable.js'),
                   'startday_template', str(today.year))

    copy(os.path.join(temp_dir, 'index.html'), os.path.realpath('..'))
    copy(os.path.join(temp_dir, 'timetable.js'), os.path.join(os.path.realpath('..'),'js'))
    copy(os.path.join(temp_dir, 'scraper.py'), os.path.join(os.path.realpath('..'),'anuscrape'))
